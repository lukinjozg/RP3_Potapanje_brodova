...
namespace Potapanjebrodova
{
    ...
    {
        private string[,] igrac_matrix = new string[10, 10];
        ...
        public Form1()
        {
            ...
            ChangeLabelsToMatchMatix(igrac_matrix);
        }
        ...
        private void PlaceBoat(int i1, int j1, int i2, int j2)
        {
            int boat_size = Math.Abs(i1 - i2 + j1 - j2);
        ...
        private void SelectBoat(int boat_index)
        {
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    ...
                    if (igrac_matrix[x,y] != "B")
                    {
                        if (CheckIfBoatCanStartHere(x, y, boats[boat_index]))
                        {
                            labels[x * 10 + y].Text = boat_index.ToString();
        ...
        private void WhereToPlaceBoat(int x, int y, int boat_index)
        {
            ...
            for(int i = 0;i < 4; i++)
            {
                ...
                if(CheckIfBoatCanBePlaced(x,y,x + smx[k],y + smy[k]))
                {
                    ...
                    LabelHandler[(x + smx[k]) * 10 + y + smy[k]] = (sender, e) => PlaceBoat(x, y, x + smx[k], y + smy[k]);
